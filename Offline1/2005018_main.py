...
#     return [int.from_bytes(key_bytes[i:i+4], byteorder='big') for i in range(0, len(key_bytes), 4)]
...
def keyExpansion(ByteKey):
    rcon = _genRcon()
    # print("Rcon values:", [f"{x:08x}" for x in rcon])
    colCnt = 4
    wCnt = 4
    Rounds = 11
    ...
    for i in range (wCnt, Rounds * colCnt): 
        temp = w[i - 1]
        if (i % wCnt == 0):
            temp = SubWord(RotWord(temp)) ^ rcon[i // wCnt]
        w[i] = w[i-wCnt] ^ temp

    ans = []
    for i in range(0, len(w), 4):
        ans.append(WordToByte(w[i:i+4]))
    return ans
...
def AES_decryption(ciphertext_bytes, allroundkeys, rounds):
    ...
    # print_matrix_inHex(state_matrix)
    allRoundKeyMatrices = [bytesToMatrix(roundKey) for roundKey in allroundkeys]
    state_matrix = addRoundKey(state_matrix, allRoundKeyMatrices[10]) # start from last round key
    # print("Initial Round") 
    # print_matrix_inHex(state_matrix)
    # print()
...
def matrixToBytes(state_matrix):
    return bytes([state_matrix[row][col] for col in range(4) for row in range(4)])
...
def aes_enc_CBC(plaintext, allroundKeys, rounds):
    ...
    iv = os.urandom(16)
    prev_block = iv
    # AES_encryption(bytes_array, roundKeys, 10)
...
key = adjustKey(key , 128)
roundKeys = keyExpansion(key)
# printKeys(roundKeys)

ciphertext_bytes = aes_enc_CBC(plaintext, roundKeys,10)
print("Ciphered text:") 
print("In HEX:", end = ' ')
...
plaintext_bytes = aes_dec_CBC(ciphertext_bytes, roundKeys, 10)
